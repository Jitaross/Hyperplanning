...
public class CoursDAO extends AbstractDAO<Cours> {
    ...
    public CoursDAO() throws DataAccessException {
        ...
        try{
            ...
            findCoursEtudiantPS = getConnection().prepareStatement("SELECT * FROM COURS WHERE DATE = ? AND IDMATIERE IN (SELECT ID FROM MATIERE WHERE IDFORMATION = (SELECT IDFORMATION FROM ETUDIANT WHERE ID =?)) And (GROUPE = NULL OR GROUPE = (SELECT GROUPE FROME ETUDIANT WHERE ID = ?))");
